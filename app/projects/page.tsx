...
export default function ProjectsPage() {
    ...
    fetch('/data/projects.json')
      ...
      .then(data => {
        const corrected = data.map((p: any) => {
          if (['luxury-three-bedroom-apartment-bole', 'minimalist-apartment-finishing-ethiopia', 'modern-studio-apartment-design-addis'].includes(p.slug)) {
  ...
  if (loading) return <div className="min-h-screen bg-white flex items-center justify-center font-black uppercase text-[10px]">Loading Portfolio...</div>;
      ...
      <main className="flex-grow pt-[80px] relative">
        <section className="pb-8 px-4 md:px-6 mt-12">
          <div className="max-w-4xl mx-auto text-center mb-8">
            <h1 className="text-4xl md:text-6xl font-black text-gray-900 mb-4 uppercase tracking-tighter">Our Projects Portfolio</h1>
            <p className="text-gray-500 max-w-2xl mx-auto text-base uppercase tracking-widest font-bold">We create workplaces that elevate your brand and inspire your people.</p>
          </div>
          ...
          <div className="sticky top-[80px] z-40 bg-white border-b border-gray-100 py-4 px-4">
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center overflow-x-auto pb-2 md:pb-0 space-x-2 scrollbar-hide">
                {categories.map(cat => (
                  <button key={cat.id} onClick={() => setFilter(cat.id)} className={`px-4 py-2 text-[10px] font-bold uppercase tracking-wide rounded-full whitespace-nowrap transition-colors ${filter === cat.id ? 'bg-black text-white' : 'text-gray-500 hover:text-gray-900 bg-gray-50'}`}>
                    {cat.label} <span className="ml-1 opacity-75">({counts[cat.id as keyof typeof counts] || 0})</span>
                  </button>
                ))}
              </div>
              <div className="relative w-full md:w-72">
                <input type="text" placeholder="Search projects..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-4 pr-4 py-2 text-[11px] border border-gray-200 rounded-full focus:ring-1 focus:ring-red-600 outline-none" />
              </div>
            </div>
          </div>
        </section>
            ...
            {filteredProjects.map((project: any, idx: number) => (
              <div key={project.slug} className="group relative aspect-[4/3] bg-gray-100 overflow-hidden">
                <Link href={`/projects/${project.slug}`} className="block w-full h-full">
                  <Image src={project.image} alt={project.title} fill priority={idx < 6} className="object-cover transition-all duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 33vw" />
                ...
                <button onClick={(e) => toggleFavorite(e, project.slug)} className="absolute top-4 left-4 w-10 h-10 flex items-center justify-center bg-black/20 backdrop-blur-md rounded-full text-white z-20 hover:bg-red-600 transition-colors">
